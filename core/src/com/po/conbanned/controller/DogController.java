...
public class DogController {
    ...
    public void update(float delta) {
        if (world.getHoverState() != World.HoverState.NONE) {
            float desiredAngleDelta = new Vector2(world.getHover()).sub(world.getDog().getPosition()).angle(world.getDog().getOrientation());
...
//            System.out.println(desiredAngleDelta + ", " + angleDelta);
            world.getDog().getOrientation().rotate(-angleDelta);

            float distance = world.getDog().getPosition().dst(world.getHover());
            ...
            Vector2 directionVector = new Vector2(world.getDog().getOrientation()).nor().scl(moveSpeed);
            world.getDog().getPosition().add(directionVector);
